function initHtml(){
    ...
    for(let i=0; i < reposRobertoSm822.length; i++){
        htmlListRepos += `<li class="list-group-item">
            ...
            <b>Último Push: </b>${localeDate(reposRobertoSm822[i].pushed_at.toLocaleString())}
        </li>`; 
...
function orderAsc(){
    let ascOrder = reposRobertoSm822.sort( function ( a, b ) { return b.name - a.name; } );
    let html = showHtmlUl(ascOrder);
...
function orderDesc(data){
    let descOrder = reposRobertoSm822.sort().reverse();
    let html = showHtmlUl(descOrder);
    jQuery("#prository-list").html(html);
    //console.log(descOrder);
    return descOrder;
}
function localeDate(data){
    var d = new Date(data); 
    //console.log( d.toLocaleString() ); 
    return d.toLocaleString();
}
function showHtmlUl(dados){
    ...
    let htmlListRepos = `<ul class="list-group">`;
        for(let i=0; i < dados.length; i++){
            htmlListRepos += `<li class="list-group-item">
                ...
                <b>Último Push: </b>${localeDate(dados[i].pushed_at)}
            </li>`; 
...
function searchRepos(_this){
    ...
    for ( var i = 0; i < keys.length; i++ ) {
        //console.log( keys[i], reposRobertoSm822[ keys[i] ] );
    ...
    let html = showHtmlUl(newRepoSearch);
    jQuery("#prository-list").html(html);
